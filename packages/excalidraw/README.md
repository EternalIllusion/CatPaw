...
```bash
npm install react react-dom @eterill/catpaw
# or
yarn add react react-dom @eterill/catpaw
```

> **Note**: If you want to try unreleased changes, use `@eterill/catpaw@next`.
...
- If imports or entrypoints are unclear, inspect `node_modules/@eterill/catpaw/package.json`. The installed package exports are the source of truth.
...
## Migrating to `@eterill/catpaw@0.18.x`

Version `0.18.x` removes the old `types/`-prefixed deep import paths. If you were importing types from `@eterill/catpaw/types/...`, switch to the new type-only subpaths below.

| Old path | New path |
| --- | --- |
| `@eterill/catpaw/types/data/transform.js` | `@eterill/catpaw/element/transform` |
| `@eterill/catpaw/types/data/types.js` | `@eterill/catpaw/data/types` |
| `@eterill/catpaw/types/element/types.js` | `@eterill/catpaw/element/types` |
| `@eterill/catpaw/types/utility-types.js` | `@eterill/catpaw/common/utility-types` |
| `@eterill/catpaw/types/types.js` | `@eterill/catpaw/types` |

Drop the `.js` extension. The new package `exports` map resolves these paths without it.

These deep subpaths are for `import type` only. Runtime imports should come from the package root, plus `@eterill/catpaw/index.css` for styles.

For example:

```ts
import { exportToSvg } from "@eterill/catpaw";
```

## Self-hosting fonts

By default, Excalidraw downloads the fonts it needs from the [CDN](https://esm.run/@eterill/catpaw/dist/prod).

For self-hosting, copy the contents of `node_modules/@eterill/catpaw/dist/prod/fonts` into the path where your app serves static assets, for example `public/`. Then set `window.EXCALIDRAW_ASSET_PATH` to that same path:
...
Read the [integration docs](https://docs.excalidraw.com/docs/@eterill/catpaw/integration).

## API

Read the [API docs](https://docs.excalidraw.com/docs/@eterill/catpaw/api).

## Contributing

Read the [contributing docs](https://docs.excalidraw.com/docs/@eterill/catpaw/contributing).
